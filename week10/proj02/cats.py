...
def accuracy(typed, reference):
    ...
    cnt = 0
    for wt, rw in zip(typed_words, reference_words):
        if wt == rw:
            cnt += 1
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    "*** YOUR CODE HERE ***"
    # !
    res = min(valid_words, key = lambda valid_word: diff_function(user_word, valid_word, limit))
    if diff_function(res, user_word, limit) <= limit:
        return res
    else:
        return user_word
    # END PROBLEM 5
...
def feline_fixes(start, goal, limit):
    ...
    elif 0 == len(start) or 0 == len(goal):
...
def fastest_words(game):
    ...
    for j in words:
        ...
        idx = 0
        for i in players:
            if all_times(game)[i][j] < mn:
                mn = all_times(game)[i][j]
                idx = i
