...
def balanced(m):
    ...
    if is_mobile(end(left(m))) and is_mobile(end(right(m))):
        ...
        return size(end(left(m))) * length(left(m)) == total_weight(end(right(m))) * length(right(m)) \
            and balanced(end(right(m)))
    elif is_mobile(end(left(m))) and is_planet(end(right(m))):
        return size(end(right(m))) * length(right(m)) == total_weight(end(left(m))) * length(left(m)) \
            and balanced(end(left(m)))
    else:
        return size(end(left(m))) * length(left(m)) == size(end(right(m))) * length(right(m))
...
def totals_tree(m):
    ...
    # !
    if is_mobile(end(left(m))) and is_mobile(end(right(m))):
        return tree(total_weight(end(left(m))) + total_weight(end(right(m))), \
            [totals_tree(end(left(m))), totals_tree(end(right(m)))])
    elif is_planet(end(left(m))) and is_mobile(end(right(m))):
        return tree(size(end(left(m))) + total_weight(end(right(m))), \
            [tree(size(end(left(m)))), totals_tree(end(right(m)))])
    elif is_mobile(end(left(m))) and is_planet(end(right(m))):
        return tree(size(end(right(m))) + total_weight(end(left(m))), \
            [totals_tree(end(left(m))), tree(size(end(right(m))))])
    else:
        return tree(size(end(right(m))) + size(end(left(m))), \
            [tree(size(end(left(m)))), tree(size(end(right(m))))])
