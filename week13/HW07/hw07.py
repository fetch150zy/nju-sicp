...
class Polynomial:
    ...
    def __add__(self, other):
        res = []
        n = min(len(self.lst), len(other.lst))
        m = max(len(self.lst), len(other.lst))
        for i in range(n):
            res.append(self.lst[i] + other.lst[i])
        if len(self.lst) > len(other.lst):
            [res.append(self.lst[i]) for i in range(n, m)]
        elif len(self.lst) < len(other.lst):
            [res.append(other.lst[i]) for i in range(n, m)]
        else:
            pass
        return Polynomial(res)
    
    def __mul__(self, other):
        res = [0 for i in range(len(self.lst) + len(other.lst) - 1)]
        k = 0
        for i, x in enumerate(self.lst):
