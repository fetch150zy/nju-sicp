...
def roll_dice(current_score, num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        if 1 == d:
            flag = True
        if (d % 2 == 0 and current_score % 2 == 0) or (d % 2 == 1 and current_score % 2 == 1):
            add_score += 1
...
def take_turn(num_rolls, current_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # !BEGIN PROBLEM 3
    if num_rolls > 0:
        current_score = roll_dice(current_score, num_rolls, dice)
    else:
...
def swine_swap(player_score, opponent_score):
    ...
    """
    # !BEGIN PROBLEM 4
    s = str(3**(player_score + opponent_score))
    if s[0] == s[-1]:
        return True
    else:
        return False
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # !BEGIN PROBLEM 7
    # 获取当前回合的分数，看看是否超过之前的回合最高分，从take_true获取到当前回合的分数，？？是否包括交换的情况？？？不知道
    def func(score0, score1):
        locale_high = running_high
        if 0 == who:
            score = score0 - last_score
            if score > locale_high:
                locale_high = score
                print(locale_high, 'point(s)! That\'s a record gain for Player 0!')
            return announce_highest(who, score0, locale_high)
        else:
            score = score1 - last_score
            if score > locale_high:
                locale_high = score
                print(locale_high, 'point(s)! That\'s a record gain for Player 1!')
            return announce_highest(who, score1, locale_high)
    return func
    ...
    #             running_high = score
    #             print(running_high, 'point(s)! That\'s a record gain for Player 0!')
    #         return announce_highest(who, score0, running_high)
    #     else:
    #         score = score1 - last_score
    #         if score > running_high:
    #             running_high = score
    #             print(running_high, 'point(s)! That\'s a record gain for Player 1!')
...
def max_scoring_num_rolls(current_score=0, dice=six_sided, trials_count=1000):
    ...
    max_rolls = 0
    for i in range(1, 11):
        res = average_dice(current_score, i, dice)
        if max_ave < res:
            max_ave = res
            max_rolls = i
    return max_rolls
